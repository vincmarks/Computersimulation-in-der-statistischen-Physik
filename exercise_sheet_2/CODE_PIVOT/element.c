...
void populate(void){
  ...
  if ((LSSUBX<RANGE)||(LSSUBY<RANGE)||(LSSUBZ<RANGE)) {
     fprintf(stderr,"ERROR in populate.c (element.c): length of subbox %g %g "
  ...
  for(ix=0;ix<BOX_LENGTH_X;ix++)            
    for(iy=0;iy<BOX_LENGTH_Y;iy++)
      for(iz=0;iz<BOX_LENGTH_Z;iz++){
  ...
  for (ibox=0,i=0;ibox<BOX_LENGTH_3;ibox++) 
    ...
    for(iy=0;iy<BOX_LENGTH_Y;iy++)
      for(iz=0;iz<BOX_LENGTH_Z;iz++){
        ...
        for(j=0;j<pbox->population;j++,philf=philf->after) { 
          ...
          if ((fmod((mon->x)/LSSUBX,(BOX_LENGTH_X))*LSSUBX < (((double)(ix))*LSSUBX))        ||
              (fmod((mon->y)/LSSUBY,(BOX_LENGTH_Y))*LSSUBY < (((double)(iy))*LSSUBY))        ||
              (fmod((mon->z)/LSSUBZ,(BOX_LENGTH_Z))*LSSUBZ < (((double)(iz))*LSSUBZ))        ||
              (fmod((mon->x)/LSSUBX,(BOX_LENGTH_X))*LSSUBX > (((double)(ix))*LSSUBX+LSSUBX)) ||
              (fmod((mon->y)/LSSUBY,(BOX_LENGTH_Y))*LSSUBY > (((double)(iy))*LSSUBY+LSSUBY)) ||
              (fmod((mon->z)/LSSUBZ,(BOX_LENGTH_Z))*LSSUBZ > (((double)(iz))*LSSUBZ+LSSUBZ))   )
